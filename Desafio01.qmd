...
```{r}
# Método 1: base R
inicio_base_r <- Sys.time()
dados <- read.csv(unz('flights.csv.zip', "flights.csv"))
final_base_r <- Sys.time()
tempo_base_r <- final_base_r - inicio_base_r
tempo_base_r
```
...
# Método 2: readr (R)
library(readr)
inicio_readr <- Sys.time()
dados <- read_csv('flights.csv.zip')
final_readr <- Sys.time()
tempo_readr <- final_readr - inicio_readr
tempo_readr
```
...
final_python = time.time()
tempo_python = final_python - inicio_python
print(tempo_python)
```
